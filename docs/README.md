...
Rec 预处理始终先将文本框高度缩放至 48；宽度超过 320 的文本框会按 320 像素、32 像素重叠的窗口分别识别。每个窗口的 CTC token 会按 time-step 映射回完整文本条带，并在相邻窗口重叠区的中点分配归属；交界处来自不同窗口的相邻相同 token 会按置信度折叠，不依赖 UTF-8 字符串前后缀匹配。空文本或平均识别置信度低于 0.50 的结果不会输出。
