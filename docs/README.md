...
Rec 预处理始终先将文本框高度缩放至 48；宽度超过 320 的文本框会按 320 像素、32 像素重叠的窗口分别识别，再按 UTF-8 字符重叠合并。空文本或平均识别置信度低于 0.50 的结果不会输出。

Det 会在 DB unclip 与坐标反变换后将四点框限制在原图边界内。页面任意边超过 `960px` 时，Det 以 `960×960`、`96px` 重叠切块执行；各块框映射回页面坐标后，以边界框 IoU ≥ 0.50 去重。小于该阈值的图片仍只执行一次 Det。
